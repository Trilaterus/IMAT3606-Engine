...
void Scene::update()
{
	...
	if (myBall != nullptr && myCube != nullptr)
	{
		if (tdcd::SphereVSphereTest(myBall->getModelPosition(), 1.0f, myCube->getModelPosition(), 1.0f))
		{
			myBall->setModelColour(1.0f, 0.0f, 0.0f);
		}
		else
		{
			myBall->setModelColour(1.0f, 1.0f, 1.0f);
		}
	}
